...
#define FHT_OSD_USER17          (0xe41d) /*user defined*/
#define FHT_OSD_USER18          (0xe41e) /*user defined*/
#define FHT_OSD_USER19          (0xe41f) /*user defined*/
#define FHT_OSD_USER20          (0xe420) /*user defined*/
#define FHT_OSD_USER21          (0xe421) /*user defined*/
#define FHT_OSD_USER22          (0xe422) /*user defined*/
#define FHT_OSD_USER23          (0xe423) /*user defined*/
#define FHT_OSD_USER24          (0xe424) /*user defined*/
#define FHT_OSD_USER25          (0xe425) /*user defined*/
#define FHT_OSD_USER26          (0xe426) /*user defined*/
#define FHT_OSD_USER27          (0xe427) /*user defined*/
#define FHT_OSD_USER28          (0xe428) /*user defined*/
#define FHT_OSD_USER29          (0xe429) /*user defined*/
#define FHT_OSD_USER30          (0xe42a) /*user defined*/
#define FHT_OSD_USER31          (0xe42b) /*user defined*/
#define FHT_OSD_USER32          (0xe42c) /*user defined*/
#define FHT_OSD_USER_MAX        (FHT_OSD_USER32)
#define FHT_OSD_TAG_CNT         (44)     /*最大支持28个TAG*/
#define FHT_OSD_MAX_CHARS_OF_TAG (64)    /*一个TAG所能包含的最大字符数*/
...
#define FHT_OSD_LAYER_TWINKLE     (FHT_OSD_TEXT_TWINKLE) /*layer中所有text line闪烁*/
#define FHT_OSD_LAYER_EXTERNAL_MEM    (1<<2) /*当前图层使用外部内存*/
...
typedef struct {
    FH_UINT64 vmm_base; /*状态，已分配的vmm内存地址*/
    FH_VOID * vmm_vbase; /*状态，已分配的vmm内存大小*/
    FH_UINT32 vmm_size; /*状态，已分配的vmm内存大小*/
}FHT_OSD_External_Mminfo_t;
...
typedef struct {
    ...
    FHT_OSD_External_Mminfo_t ext_mminfo;
...
/*
 * 功能: 计算对应幅面的宽高和位宽模式所需的vmm内存大小
 * 输入参数: width: 需要计算的幅面宽度
 *      height,需要计算的幅面高度
 *       pixel, 对应的osd所使用的位宽模式,可配置1/2/4/8/16/32
 * 输出参数: 返回当前幅面大小和位宽模式所需的vmm内存大小
 * 说明: TOSD的pixel固定为2;
 *       GBOX和MASK可动态配置,默认为1,默认双buf无法修改;
 *       LOGO为RGB1555模式,pixel为16,默认单buf无法修改;
 *       Circle默认pixel为1无法修改,默认单buf无法修改;
 * 另外双buf模式下需要在函数外部手动以返回值*2为真实大小.
 */
FH_UINT32 FHAdv_Osd_VmmSize_Calc(FH_UINT32 width, FH_UINT32 height, FH_UINT32 pixel);
#ifdef __cplusplus
#if __cplusplus
}
#endif
