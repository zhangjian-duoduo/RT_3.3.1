...
#define FH_OBJDET_NOT_SUPPORT              (FH_OBJDET_ERR_BASE -28)
#define FH_OBJDET_DEVICE_FAILED            (FH_OBJDET_ERR_BASE -29)
...
typedef enum FH_DET_MODE {
    FH_OBJDET_FULL_FRAME = 0,       /*全幅面模式，返回物体个数以及坐标*/
    FH_OBJDET_ROI_ONLY   = 1,          /*ROI模式，返回是否包含被测物体*/
    DUMMY_DETECT_MODE    = 0X10000
}FH_DET_MODE_t;/*检测模式枚举*/

typedef enum FH_MODEL_SUPPORTED_MODE {
    FH_MODE_TIME_SAVING        = 0,             /* FH0.5 mode with least time cost */
    FH_MODE_PERFORMACE_INHANCE = 1,      /* FH0.6 mode with 5% more cost than V05, but better detect performance*/
    FH_MODE_YS_MODELS          = 2,               /* FH0.7 mode for YS models*/
    FH_MODE_SIMPLE_YS_MODELS   = 3,         /* FH0.7 simple mode for YS models*/
    DUMMY_MODEL_MODE = 0X10000
 ...
 *                          其他：错误码
 */
FH_SINT32 VB_ODET_MODEL_setThresh(struct FH_ODET_Handle *det, FH_UINT32 modelId, FH_SINT32 thresh);
 ...
 *                          其他：错误码
 */
FH_SINT32 VB_ODET_setSampleWinPara(struct FH_ODET_Handle *det, FH_UINT8 perScaleRate, FH_UINT8 maxScaleNum, FH_UINT8 slideStep);
/**
 * 创建、初始化跟踪器
 *
 * input:   *img            图像输入
 ...
 * return:  FH_VOID
 */
FH_CHAR *VB_ODET_Version(FH_SINT32 print_enable);
