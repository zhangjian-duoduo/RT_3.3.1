...
/**
 * 注意:此处的质量或码率优先，主要是针对单帧相同QP的情况上相比较，并不意味着码流最终的效果上码率优先策略一定会比质量优先差.
 * 因为质量优先为了提高一些针对性场景或图像会使用更多码率，如果在相同目标码率的码控控制下，码率优先策略因为消耗码率低从而
 * 相对的帧级QP更小, 可能会存在部分效果上反而优于质量优先的情况!!
*/
typedef enum
{
