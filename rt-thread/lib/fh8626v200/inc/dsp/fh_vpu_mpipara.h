...
#define MAX_MASK_EDGE                        (10)
#define MAX_VPU_CHANNO                       (3)
...
typedef struct
{
	...
	FH_VPU_CROP             crop_area;    // 通道裁剪区域(不支持) | [ ]
}FH_VPU_SIZE;
...
typedef struct
{
	...
	FH_UINT32               depth;        // 通道输出队列深度(需要小于通道buffer个数,为0时不允许使用norpt获取yuv,为0且使用vb时获取yuv的timeout不能为0) | [0-6]
}FH_VPU_CHN_CONFIG;
...
{
	FH_UINT32              masaic_enable; // 隐私屏蔽形式 0:单色 1:马赛克 | [0-1]
	FH_UINT32              masaic_size;   // 马赛克单块大小 0:8x8,1:16x16,2:32x32,3:64x64 | [0-3]
	FH_UINT32              trsluct;       // 屏蔽区域半透明控制参数 0:半透明,15:不透明 | [0-15]
	FH_UINT32              msaa_strength; // 抗锯齿强度控制参数(推荐7-9)  | [1-15]
}FH_MASK_MASAIC_V2;

typedef struct
{
	FH_UINT32               mask_area;                   // 隐私屏蔽区域 | [0-7]
	FH_BOOL                 mask_enable;                 // 隐私屏蔽使能 | [0-1]
	FH_UINT32               edge_value;                  // 隐私屏蔽区域边数 | [3-10]
	FH_POINT                point_value[MAX_MASK_EDGE];  // 隐私屏蔽区域坐标 | [ ]
	FH_UINT32               color;                       // 隐私显示颜色(格式AYCbCr,Alpha无效,多区域共用) | [ ]
	FH_MASK_MASAIC_V2       masaic;                      // 马赛克属性(多区域共用) | [ ]
}FH_VPU_MASK_V2;
...
typedef struct
{
	...
	FH_UINT32      vi_auto;        // 驱动是否对visize进行自适应更改(使用拼接时要关闭) | [0-1]
}FH_VPU_GRP_PARAM;
