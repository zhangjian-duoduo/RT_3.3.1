...
static long _list_thread(struct rt_list_node *list)
{
    ...
    do
    {
        ...
        for (i = 0; i < find_arg.nr_out; i++)
        {
            ...
            thread = (struct rt_thread *)&thread_info;
#ifdef RT_USING_SMP
            if (thread->oncpu != RT_CPU_DETACHED)
                rt_kprintf("%-*.*s %3d %3d %3d ", maxlen, RT_NAME_MAX, thread->name, thread->oncpu, thread->bind_cpu, thread->current_priority);
            else
                rt_kprintf("%-*.*s N/A %3d %3d ", maxlen, RT_NAME_MAX, thread->name, thread->bind_cpu, thread->current_priority);
#else
...
static long _list_thread(struct rt_list_node *list)
{
    ...
    rt_kprintf(" --- ---- ---  ------- ---------- ----------  ------  ---------- ---\n");
#else
    rt_kprintf("%-*.s pri  status      sp     stack size max used left tick  error   function\n", maxlen, item_title);
    object_split(maxlen);
    rt_kprintf(" ---  ------- ---------- ----------  ------  ---------- ---   --------\n");
#endif
    do
    {
        ...
        for (i = 0; i < find_arg.nr_out; i++)
        {
            ...
            rt_kprintf(" 0x%08x 0x%08x    %02d%%   0x%08x %03d   ",
                    ...
                    thread->error);
#ifndef RT_USING_VFP
            if (thread->sp == (rt_thread_self()->sp))
                asm("mov %0, fp":"=r"((rt_uint32_t)fp));
            else
                fp = (rt_uint32_t *)(*((rt_uint32_t *)(thread->sp) + 12));
#else
            if (thread->sp == (rt_thread_self()->sp))
                asm("mov %0, fp":"=r"((rt_uint32_t)fp));
            else
                fp = (rt_uint32_t *)(*((rt_uint32_t *)(thread->sp) + 12 + 32));
#endif
            while (1)
            {
                if ((rt_uint32_t)fp < FH_DDR_START + 0xc || (rt_uint32_t)fp > FH_DDR_END)
                    break;
                pc = *fp - 0xC;
                fp = (rt_uint32_t *)(*(fp - 3));
                if (pc == (rt_uint32_t)thread->entry || (rt_uint32_t)fp == 0x0)
                {
                    breaks++;
                    rt_kprintf("0x%x<-\n", pc);
                    break;
...
static void os_list_usage()
{
    rt_kprintf("show os object information\n\n");
    rt_kprintf("Usage:\n");
    rt_kprintf("  os_list -h: show this help\n");
    rt_kprintf("  os_list -t: show system threads\n");
    rt_kprintf("  os_list -d: show system devices\n");
    rt_kprintf("  os_list -c: show system timers\n");
    rt_kprintf("  os_list -m: show system mempool(s)\n");
    rt_kprintf("  os_list -p: show system memheap(s)\n");
    rt_kprintf("  os_list -q: show system msgqueue\n");
    rt_kprintf("  os_list -b: show system mailbox(s)\n");
    rt_kprintf("  os_list -x: show system mutex(s)\n");
    rt_kprintf("  os_list -e: show system event(s)\n");
    rt_kprintf("  os_list -s: show system semaphore(s)\n");
    rt_kprintf("  os_list -k: show system clock config\n");
    rt_kprintf("  os_list -i: show system interrupts\n");
#ifdef RT_USING_DFS
    rt_kprintf("  os_list -f: show system open file(s)\n");
#endif
}

extern void list_irq(void);
int cmd_os_list(int argc, char *argv[])
{
    if (argc < 2)
    {
        os_list_usage();
    ...
    if (rt_strcmp(argv[1], "-q") == 0)
    {
        list_msgqueue();
        return 0;
    }
    if (rt_strcmp(argv[1], "-b") == 0)
    {
        list_mailbox();
        return 0;
    }
    if (rt_strcmp(argv[1], "-x") == 0)
    {
        list_mutex();
        return 0;
    }
    ...
    if (rt_strcmp(argv[1], "-k") == 0)
    {
        extern void fh_clk_debug();
        fh_clk_debug();
        return 0;
    }
#ifdef RT_USING_DFS
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_os_list, __cmd_os_list, list os kernel object info);
