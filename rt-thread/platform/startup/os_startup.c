...
void print_os_startup_time(void)
{
    rt_kprintf("           OS startup time: % 10u\n", g_os_startup_time);
}
#endif
...
#ifdef FH_FAST_BOOT
static void _do_fast_boot_init(void)
{
    volatile unsigned int *pgpio0;

    pgpio0 = (volatile unsigned int *)GPIO0_REG_BASE;
    pgpio0[0] |= 1 << CONFIG_CIS_RESET_GPIO;
}
#endif
...
#ifdef CP_DO_LOAD_CODE
    extern void start_load_arc(void);
    start_load_arc();
#endif
