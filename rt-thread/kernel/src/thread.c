...
#ifdef RT_USING_THREAD_LOCK
    thread->lock = rt_sem_create(name, 0, 0);
#endif
...
#ifdef RT_USING_THREAD_LOCK
rt_sem_t *rt_thread_get_sem_lock(void)
{
    rt_thread_t cur_thread;
    cur_thread = rt_thread_self();
    if (cur_thread != RT_NULL)
    {
        return (rt_sem_t *)(&cur_thread->lock);
    }
    else
    {
        return RT_NULL;
...
static void smp_thrd(void *param)
{
    while (1)
    {
        int cid = rt_hw_cpu_id() & 3;
        rt_thread_delay(200 + cid * 100);
        rt_kprintf("SMP thread[%s] running on CPU %d@%d\n", rt_thread_self()->name, cid, rt_tick_get());
    }
}

void test_smp_thread(int cid)
{
    char tname[20];

    rt_sprintf(tname, "smp_%d", cid);
    rt_thread_t tid = rt_thread_create(tname, smp_thrd, RT_NULL,
                                       0x4000,  20, 10);
    if (tid != RT_NULL)
    {
        rt_thread_control(tid, RT_THREAD_CTRL_BIND_CPU, (void *)cid);

        rt_thread_startup(tid);
    }
    else
    {
        rt_kprintf("create smp thread failed.\n");
    }
}
#endif
