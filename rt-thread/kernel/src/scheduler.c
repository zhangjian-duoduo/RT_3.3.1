...
static rt_uint32_t __sys_total_ticks = 0;
static rt_uint32_t __idl_total_ticks = 0;
static rt_uint32_t __prev_calc_ticks = 0;
...
void rt_cpuload_get(rt_uint32_t *idle, rt_uint32_t *total)
{
    int lvel = rt_hw_interrupt_disable();
    rt_uint32_t ctick = (rt_uint32_t)read_pts();

    *total = (rt_uint32_t)((rt_int32_t)ctick - (rt_int32_t)__sys_total_ticks);
    __sys_total_ticks = ctick;
    *idle = __idl_total_ticks;
    __idl_total_ticks = 0;
    rt_hw_interrupt_enable(lvel);
}
...
static void _rt_scheduler_stack_check(struct rt_thread *thread)
{
    ...
    if (*((rt_uint8_t *)((rt_uint32_t)thread->stack_addr + thread->stack_size - 1)) != '#' ||
#else
...
#ifdef RT_USING_SMP
    to_thread->oncpu = rt_hw_cpu_id() & 3;
#else
...
#ifdef RT_DETECT_INFINITE_LOOP
static unsigned int __sys_busy = 0;
static unsigned int __glastick = 0;
static unsigned int __busyprio = 0;
static char __busythrd[RT_NAME_MAX] = { '\0' };
#endif
void rt_schedule(void)
{
    rt_base_t level;
    ...
    if (rt_scheduler_lock_nest == 0)
    {
...
#ifdef RT_DETECT_INFINITE_LOOP
        if (to_thread->current_priority != RT_THREAD_PRIORITY_MAX - 1)
        {
            if (rt_tick_get() != __glastick)
            {
                __glastick = rt_tick_get();
                __sys_busy++;
                if (__sys_busy > RT_INFINITE_LOOP_THRESHOLD)
                {
                    rt_kprintf("[ERROR] Infinite Loop detected! thread priority: [%d], name: [%s]\n",
                                __busyprio, __busythrd);
                    __sys_busy = 0;
                    __busyprio = 0;
                }
            }
            if (__sys_busy > RT_INFINITE_LOOP_THRESHOLD/2)
            {
                if (to_thread->current_priority > __busyprio)
                {
                    __busyprio = to_thread->current_priority;
                    rt_strncpy(__busythrd, to_thread->name, RT_NAME_MAX);
                }
            }
        }
        else
        ...
        if (to_thread != rt_current_thread)
        {
            ...
            if (from_thread->current_priority == RT_THREAD_PRIORITY_MAX - 1)
            {
                __idl_total_ticks += (rt_int32_t)(rt_uint32_t)read_pts() - (rt_int32_t)__prev_calc_ticks;
            }
            if (to_thread->current_priority == RT_THREAD_PRIORITY_MAX - 1)
            {
                __prev_calc_ticks = (rt_uint32_t)read_pts();
            }
            RT_OBJECT_HOOK_CALL(rt_scheduler_hook, (from_thread, to_thread));
            RUNLOG_ADD(__LINE__, (void *)to_thread, "Thead", to_thread->name);
