...
static struct dfs_fd *_fds_table[DFS_FD_MAX];
...
int dfs_init(void)
{
    ...
    /* Initialize static fds_table */
    _fdtab.fds = &_fds_table[0];
    /* Max alloc fd = DFS_FD_MAX - DFS_FD_OFFSET
     * Reserved DFS_FD_OFFSET(3) for Console cant manage */
#if defined(RT_USING_DFS_DEVFS) && defined(RT_USING_CONSOLE)
    _fdtab.maxfd = DFS_FD_MAX - DFS_FD_OFFSET;
#else
    _fdtab.maxfd = DFS_FD_MAX;
#endif
...
#ifdef RT_USING_DFS_DEVFS
    {
        ...
        if (dfs_mount(NULL, "/dev", "devfs", 0, 0) == 0)
        {
            LOG_I("Device File System initialized!\n");
        }
        else
        {
            LOG_E("Device File System initialzation failed!\n");
            return -1;
        }
    }
#endif
...
static int fd_alloc(struct dfs_fdtable *fdt, int startfd)
{
    ...
    do
    {
        ...
        if (idx >= fdt->maxfd)
            idx -= fdt->maxfd;
    } while (idx != g_current_fd);


    /* allocate 'struct dfs_fd' */
    if (idx != g_current_fd && fdt->fds[idx] == RT_NULL)
    {
...
int check_uninstall_fs_valid(char *fs_path)
{
    struct dfs_fd *fd;
    int index;
    struct dfs_fdtable *fdt;
    ...
    for (index = 0; index < fdt->maxfd; index++)
    {
        ...
        if (fd->fs == RT_NULL)
            continue;
        if (fd->fs->ops == RT_NULL)
            continue;
        if (fd->fs->path == RT_NULL)
            continue;
        if (!rt_strcmp(fd->fs->path, fs_path))
        {
            rt_kprintf("Umount Filesysytem Valid!\n");
            rt_kprintf("Valid fd path:%s no:%d\n", fd->path, fd);
            dfs_unlock();
            return -RT_EBUSY;
        }
        else
...
int show_filesystem_state(void)
{
        ...
        iter < &filesystem_table[DFS_FILESYSTEMS_MAX]; iter++)
    {
        if (iter->ops == NULL)
            ...
            rt_kprintf("%s on path:%s device:%s type %s\n", iter->ops->name,
            iter->path, iter->dev_id, iter->ops->name);
    }
    ...
    for (index = 0; index < fd_table->maxfd; index++)
    {
        ...
        if (fd && fd->fops)
        {
            ...
            if (fd->type == FT_DIRECTORY)
                ...
                rt_kprintf("%-7.7s ", "file");
            else if (fd->type == FT_SOCKET)
                rt_kprintf("%-7.7s ", "socket");
            else if (fd->type == FT_USER)
                rt_kprintf("%-7.7s ", "user");
            else
                rt_kprintf("%-8.8s ", "unknown");
...
/* MSH_CMD_EXPORT(list_fd, list file descriptor);   */  /* merged into os_list */
FINSH_FUNCTION_EXPORT(list_fd, list file descriptor);
#endif
/*@}*/
