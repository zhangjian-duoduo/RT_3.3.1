...
#if defined(RT_USING_NEWLIB) && defined(_EXFUN)
_READ_WRITE_RETURN_TYPE _EXFUN(readv, (int fd, const struct iovec *iov, int iovcnt))
#else
...
#if defined(RT_USING_NEWLIB) && defined(_EXFUN)
_READ_WRITE_RETURN_TYPE _EXFUN(writev, (int fd, const struct iovec *iov, int iovcnt))
#else
...
int ftruncate(int fd, off_t len)
{
    ...
    result = dfs_file_ftruncate(d, len);
    if (result < 0)
    {
        fd_put(d);
        rt_set_errno(-result);
...
int fhide(int fd, int enable)
{
    ...
    result = dfs_file_fhide(d, enable);
    if (result < 0)
    {
        fd_put(d);
        rt_set_errno(-result);
...
rt_off64_t lseek64(int fd, rt_off64_t offset, int whence)
{
    rt_off64_t result;
    struct dfs_fd *d;
    ...
    result = dfs_file_lseek64(d, offset);
    if ((signed long long)result < 0)
    {
        fd_put(d);
        rt_set_errno(-result);
...
int fstat(int fildes, struct stat *buf)
{
    ...
    if (d->fs->ops->stat != RT_NULL)
        d->fs->ops->stat(d->fs, d->path, buf);
#endif
...
int fcntl(int fildes, int cmd, ...)
{
    ...
    if (fildes >= DFS_FD_MAX)
    {
        ...
        val = va_arg(args, int);
        va_end(args);
        return lwip_fcntl(fildes, cmd, val);
    }
#endif
...
int ioctl(int fildes, int cmd, ...)
{
    ...
    if (fildes >= DFS_FD_MAX)
    {
        ...
        arg = va_arg(ap, void*);
        va_end(ap);
        return lwip_ioctl(fildes, cmd, arg);
    }
#endif
...
int mkdir(const char *path, mode_t mode)
{
    ...
    if (strlen(path) > 255)
        return -1;
    name = rt_malloc(strlen(path) + 1);
    memcpy(name, path, strlen(path) + 1);
    /* determine whether the path is an absolute path */
    if (name[0] != '/')
        flag = 1;

    /* combine string as path*/
    while ((file = strtok_r(RT_NULL, "/", &name)) != RT_NULL)
    {
        if (flag == 1)
            flag = 0;
        else
        ...
        d = fd_get(fd);
        result = dfs_file_open(d, spilt, O_DIRECTORY);
        if (result < 0)
        {
            result = dfs_file_open(d, spilt, O_DIRECTORY | O_CREAT);
    ...
    {
        ret = -EEXIST;
        rt_kprintf("%s:can't create directory %s: File exists\n", __func__, path);
    }
    else
        ret = RT_EOK;
...
int mount(const char   *device_name,
            const char   *path,
            ...
            const void   *data)
{
    ...
    rt_memset(dev_name, 0, 16);
    if (device_name != RT_NULL)
    {
        p = strstr(device_name, "/dev/");
        if (p != RT_NULL)
        {
            p += rt_strlen("/dev/");
            rt_memcpy(dev_name, p, rt_strlen(device_name) - rt_strlen("/dev/"));
        }
        else
            rt_strncpy(dev_name, device_name, 16);
        dev = &dev_name[0];
    }
    else
        dev = (char *)device_name;

    if (filesystemtype != RT_NULL)
    {
        if (rt_strcmp(filesystemtype, "vfat") == 0)
            rt_strncpy(fs_type, "elm", sizeof("elm"));
        else
            rt_strncpy(fs_type, filesystemtype, 16);
    }
    else
    {
        rt_set_errno(EINVAL);
    ...
    return dfs_mount(dev, path, fs_type, rwflag, data);
}

RTM_EXPORT(mount);
#ifdef RT_USING_FINSH
FINSH_FUNCTION_EXPORT(mount, mount a file system);
#endif

int umount(const char *specialfile)
{
    return dfs_unmount(specialfile);
}

RTM_EXPORT(umount);
#ifdef RT_USING_FINSH
FINSH_FUNCTION_EXPORT(umount, unmount a file system);
#endif
