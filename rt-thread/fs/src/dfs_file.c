...
int dfs_file_open(struct dfs_fd *fd, const char *path, int flags)
{
    ...
    if (fs->flags & DFS_MS_RDONLY)
    {
        if (flags & (O_ACCMODE|O_CREAT|O_TRUNC|O_APPEND))
        {
            LOG_E("file system is mount as read only, no write permited: %s\n", path);
            return -EROFS;
        }
    }
...
int dfs_file_close(struct dfs_fd *fd)
{
    ...
    rt_uint16_t type = fd->type;

    /* just lock/unlock fs for regular file */
    if (type == FT_REGULAR)
        dfs_lock();
    if (fd != NULL && fd->fops && fd->fops->close != NULL)
        result = fd->fops->close(fd);

    if (type == FT_REGULAR)
        dfs_unlock();

    /* if close fd error, free fd->path
     * or Memory leak
     */

    if (result)
    {
        LOG_E("!!!!!!!!! close file \"%s\" fail\n", fd->path);
    }
...
/**
 * this function will perform a io fcntl on a file descriptor.
 *
 * @param fd the file descriptor.
 * @param cmd the command to send to file descriptor.
 * @param args the argument to send to file descriptor.
 ...
 * @return 0 on successful, -1 on failed.
 */
int dfs_file_fcntl(struct dfs_fd *fd, int cmd, void *args)
{
    if (fd == NULL)
        return -EINVAL;
...
int dfs_file_unlink(const char *path)
{
    ...
    if (fs->flags & DFS_MS_RDONLY)
    {
        /* filesystem is mount only */
        LOG_E("try to unlink a file on a read-only filesystem.\n");
        result = -EROFS;
...
int dfs_file_write(struct dfs_fd *fd, const void *buf, size_t len)
{
    ...
    if (fd->fs && (fd->fs->flags & DFS_MS_RDONLY))
    {
        /* filesystem is mount only */
        LOG_E("try to write on a read-only filesystem: %s\n", fd->path);
        return -EROFS;
    }
...
int dfs_file_flush(struct dfs_fd *fd)
{
    ...
    {
        /* filesystem is mount only */
        LOG_E("try to write on a read-only filesystem(flush): %s\n", fd->path);
        return -EROFS;
    }
    if (fd->fops == NULL)
        return -ENOENT;
...
int dfs_file_ftruncate(struct dfs_fd *fd, rt_off_t len)
{
    if (fd == NULL)
        return -EINVAL;
    ...
    {
        /* filesystem is mount only */
        LOG_E("try to write on a read-only filesystem(truncate): %s\n", fd->path);
        return -EROFS;
    }
    if (fd->fops == NULL)
        return -ENOENT;
    ...
    return fd->fops->ftruncate(fd, len);
}

int dfs_file_fhide(struct dfs_fd *fd, int enable)
{
    if (fd == NULL)
        return -EINVAL;
    ...
    if (fd->fops->fhide == NULL)
        return -ENOSYS;

    return fd->fops->fhide(fd, enable);
}
...
rt_off64_t dfs_file_lseek64(struct dfs_fd *fd, rt_off64_t offset)
{
    rt_off64_t result;
    ...
    if (fd->fops->lseek64 == NULL)
        return -ENOSYS;

    result = fd->fops->lseek64(fd, offset);
...
int dfs_file_rename(const char *oldpath, const char *newpath)
{
    ...
    if (newfs->flags & DFS_MS_RDONLY)
    {
        /* filesystem is mount only */
        LOG_E("try to write on a read-only filesystem(rename): %s\n", newpath);
        result = -EROFS;
...
int touch(char *filename)
{
    ...
    if (filename == NULL)
    {
        rt_kprintf("Usage: touch(\"filename\")");
        return -1;
    }
    fd = open(filename, O_CREAT | O_RDWR | O_APPEND, 0);
    if (fd < 0)
    {
        rt_kprintf("touch a file failed\n");
        return -1;
    }
    close(fd);
    return RT_EOK;
}
FINSH_FUNCTION_EXPORT(touch, create a file)
...
FINSH_FUNCTION_EXPORT_ALIAS(cmd_touch, __cmd_touch, touch file);
