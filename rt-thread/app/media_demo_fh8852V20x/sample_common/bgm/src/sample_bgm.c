...
FH_SINT32 sample_common_bgm_init()
{
    ...
    for (grploop = 0; grploop < MAX_GRP_NUM; grploop++)
    {
        ...
        if (grp_info.enable && grp_info.bgm_enable)
        {
            ...
            init_mem_w = ALIGNTO(vpu_info.max_width, 16) / grp_info.bgm_ds;
            init_mem_h = ALIGNTO(vpu_info.max_height, 16) / grp_info.bgm_ds;
            ...
            picsize.u32Width = ALIGNTO(vpu_info.width, 16) / grp_info.bgm_ds;
            picsize.u32Height = ALIGNTO(vpu_info.height, 16) / grp_info.bgm_ds;
