#ifndef __SENSOR_PWM_H__
#define __SENSOR_PWM_H__

#define MAX_PWM_CHANNUM (8)

typedef struct
{
    int groupid;
    int pwm_cnt;
    unsigned int freq;
    unsigned char chn_param[MAX_PWM_CHANNUM];
} pwm_motor_config_t;

typedef struct
{
    int groupid;
    int pwm_cnt;
    int period_ns[MAX_PWM_CHANNUM];
    int duty_ns[MAX_PWM_CHANNUM];
    int phase_ns[MAX_PWM_CHANNUM];
} pwm_motor_config2_t;

typedef unsigned int __u32;

#define DEVICE_NAME "/dev/fh_pwm"
#define FH_PWM_PROC_FILE "driver/pwm"

#define REG_PWM_CTRL (0x00)
#define REG_PWM_CMD(n) (((n)*4) + REG_PWM_CTRL + 4)

#define PWM_IOCTL_MAGIC 'p'
#define ENABLE_PWM _IOWR(PWM_IOCTL_MAGIC, 0, __u32)
#define DISABLE_PWM _IOWR(PWM_IOCTL_MAGIC, 1, __u32)

#define SET_PWM_DUTY_CYCLE _IOWR(PWM_IOCTL_MAGIC, 2, __u32)
#define GET_PWM_DUTY_CYCLE _IOWR(PWM_IOCTL_MAGIC, 3, __u32)
#define SET_PWM_DUTY_CYCLE_PERCENT _IOWR(PWM_IOCTL_MAGIC, 4, __u32)
#define SET_PWM_ENABLE _IOWR(PWM_IOCTL_MAGIC, 5, __u32)
#define ENABLE_MUL_PWM _IOWR(PWM_IOCTL_MAGIC, 6, __u32)
#define DISABLE_MUL_PWM _IOWR(PWM_IOCTL_MAGIC, 11, __u32)
#define WAIT_PWM_FINISH _IOWR(PWM_IOCTL_MAGIC, 12, __u32)
#define PWM_IOCTL_MAXNR 16

struct fh_pwm_config
{
    unsigned int period_ns; // pwm整个周期
    unsigned int duty_ns;
    unsigned int pulses;
#define FH_PWM_STOPLVL_LOW (0x0)
#define FH_PWM_STOPLVL_HIGH (0x3)
#define FH_PWM_STOPLVL_KEEP (0x1)

#define FH_PWM_STOPCTRL_ATONCE (0x00)
#define FH_PWM_STOPCTRL_AFTERFINISH (0x10)
    unsigned int stop;
    unsigned int delay_ns;
    unsigned int phase_ns;
    unsigned int percent;
    unsigned int finish_once;
    unsigned int finish_all;
};

struct fh_pwm_status
{
    unsigned int done_cnt;
    unsigned int total_cnt;
    unsigned int busy;
    unsigned int error;
};

typedef struct fh_pwm_chip_data
{
    int id;
    struct fh_pwm_config config;
    struct fh_pwm_status status;
    void (*finishall_callback)(struct fh_pwm_chip_data *data);
    void (*finishonce_callback)(struct fh_pwm_chip_data *data);
}PWM_CONFIG_DATA;


typedef struct fh_pwm_conf
{
    unsigned int id;
    unsigned int period_ns; // pwm整个周期
    unsigned int duty_ns;
    unsigned int delay_ns;
    unsigned int phase_ns;
    unsigned int pulses;
}FH_PWM_CONF;

void FH_PWM_Init(int pwmId);
void FH_PWM_setConfig(FH_PWM_CONF *pwm_conf);
void FH_PWM_Start(void);
void FH_PWM_Stop(void);
void FH_PWM_RateChange(char *rate);
void FH_TimerInit(unsigned int microseconds);
void FH_SensorSequeCreate_Handler(int signum);

#endif /* __SENSOR_PWM_H__ */
