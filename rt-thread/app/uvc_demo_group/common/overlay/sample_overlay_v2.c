...
/* FH8636和FH8852V20X共有4个图层，可任意配置在通道前或者各个通道上 */
/* FH885xV210共有20个GOSD图层，通道前2个，三个通道上各6个 */
...
static FH_SINT32 sample_osd_init(FH_VOID)
{
    ...
    /* 初始化OSD */
    ret = FHAdv_Osd_Init(FH_GRP_ID, FHT_OSD_DEBUG_LEVEL_ERROR, graph_ctrl, 0, 0);
    if (ret != FH_SUCCESS)
    {
...
static FH_SINT32 sample_set_channel_tosd(FH_SINT32 vpuid, FH_SINT32 channel)
{
    ...
    pOsdLayerInfo[0].layerId = channel;
    ...
    FHT_OSD_Ex_TextLine_t pTextCfg;
    pTextCfg.LineNum = 1;
    /* 设置text行块个数 */
    pTextCfg.Reconfig = 1;
    pTextCfg.pLineCfg = &text_line_cfg[0];
    ret = FHAdv_Osd_Ex_SetTextLine(vpuid, channel, pOsdLayerInfo[0].layerId, &pTextCfg);
    if (ret != FH_SUCCESS)
    {
        if (channel == 0) /*TOSD可能位于分通道前,这样channel 1可能没有,因此就不打印出错*/
        {
...
static FH_SINT32 sample_set_tosd(FH_VOID)
{
    ...
    struct vpu_channel_info chinfo;
    ...
    FHAdv_Osd_SetTagCallback(FH_GRP_ID, FHT_OSD_USER1+0, FHT_UPDATED_SECOND, sample_osd_char_traverse_callback, sample_osd_char_refresh_callback);
    FHAdv_Osd_SetTagCallback(FH_GRP_ID, FHT_OSD_USER1+1, FHT_UPDATED_SECOND, sample_osd_char_traverse_callback, sample_osd_char_refresh_callback);

    /*先判断,如果channel 0 enable,那么配置channel 0 TOSD*/
    sample_common_dsp_get_vpu_chn_info(FH_GRP_ID, 0, &chinfo);
    if (chinfo.enable)
    {
        sample_set_channel_tosd(FH_GRP_ID, 0);
...
static FH_SINT32 sample_set_logo(FH_VOID)
{
    ...
    ret = FHAdv_Osd_SetLogo(FH_GRP_ID, &logo_cfg);
    if (ret != FH_SUCCESS)
    {
...
static FH_SINT32 sample_set_mask(FH_VOID)
{
    ...
    ret = FHAdv_Osd_SetMask(FH_GRP_ID, &mask_cfg);
    if (ret != FH_SUCCESS)
    {
...
FH_SINT32 sample_set_gbox(FH_UINT32 chan, FH_UINT32 enable,  FH_UINT32 box_id, FH_UINT32 x, FH_UINT32 y, FH_UINT32 w, FH_UINT32 h, FHT_RgbColor_t color)
{
    ...
    gbox_cfg.max_frame_w = 4096;
    gbox_cfg.max_frame_h = 2196;
    ret = FHAdv_Osd_Ex_SetGbox(FH_GRP_ID, chan, &gbox_cfg);
    if (ret != FH_SUCCESS)
    {
