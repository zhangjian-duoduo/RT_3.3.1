...
int dmc_pes_subscribe(int pesgrp, char* ip, int port)
{
    int i;
    int ret;
    int code;
    ...
    ret = srt_libpes_init(port, pesgrp);
#else
    ret = libpes_init();
#endif
    if (ret != 0)
    ...
    dmc_subscribe("PES", pesgrp, DMC_MEDIA_TYPE_H264 | DMC_MEDIA_TYPE_H265, _pes_input_fn);
