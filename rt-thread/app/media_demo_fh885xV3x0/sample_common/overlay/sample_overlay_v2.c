...
/* FH8898有20个GOSD图层，通道前2个，三个通道上各6个 */
...
static FH_SINT32 sample_osd_init(FH_VOID)
{
    ...
    /* 初始化OSD */
    ret = FHAdv_Osd_Init(FH_APP_GRP_ID, FHT_OSD_DEBUG_LEVEL_ERROR, graph_ctrl, 0, 0);
    if (ret != FH_SUCCESS)
    {
...
static FH_SINT32 sample_set_channel_tosd(FH_SINT32 vpuid, FH_SINT32 channel)
{
    ...
    ret = FHAdv_Osd_SetTextLine(vpuid, channel, pOsdLayerInfo[0].layerId, &text_line_cfg[0]);
    if (ret != FH_SUCCESS)
    {
        if (channel == 0) /*TOSD可能位于分通道前,这样channel 1可能没有,因此就不打印出错*/
        {
...
static FH_SINT32 sample_set_tosd(FH_VOID)
{
    ...
    FHAdv_Osd_SetTagCallback(FH_APP_GRP_ID, FHT_OSD_USER1+0, FHT_UPDATED_SECOND, sample_osd_char_traverse_callback, sample_osd_char_refresh_callback);
    FHAdv_Osd_SetTagCallback(FH_APP_GRP_ID, FHT_OSD_USER1+1, FHT_UPDATED_SECOND, sample_osd_char_traverse_callback, sample_osd_char_refresh_callback);

    /*先判断,如果channel 0 enable,那么配置channel 0 TOSD*/
    sample_common_dsp_get_vpu_chn_info(FH_APP_GRP_ID, 0, &chinfo);
    if (chinfo.enable)
    {
        sample_set_channel_tosd(FH_APP_GRP_ID, 0);
    }

    /*先判断,如果channel 1 enable,那么配置channel 1 TOSD*/
    sample_common_dsp_get_vpu_chn_info(FH_APP_GRP_ID,1,&chinfo);
    if (chinfo.enable)
    {
        ...
        sample_set_channel_tosd(FH_APP_GRP_ID, 0);
#else
        sample_set_channel_tosd(FH_APP_GRP_ID, 1);
#endif
    }
...
static FH_SINT32 sample_set_logo(FH_VOID)
{
    ...
    ret = FHAdv_Osd_SetLogo(FH_APP_GRP_ID, &logo_cfg);
    if (ret != FH_SUCCESS)
    {
...
static FH_SINT32 sample_set_mask(FH_VOID)
{
    ...
    ret = FHAdv_Osd_SetMask(FH_APP_GRP_ID, &mask_cfg);
    if (ret != FH_SUCCESS)
    {
...
FH_SINT32 sample_set_gbox(FH_UINT32 chan, FH_UINT32 enable,  FH_UINT32 box_id, FH_UINT32 x, FH_UINT32 y, FH_UINT32 w, FH_UINT32 h, FHT_RgbColor_t color)
{
    ...
    ret = FHAdv_Osd_Ex_SetGbox(FH_APP_GRP_ID, chan, &gbox_cfg);
    if (ret != FH_SUCCESS)
    {
...
FH_VOID sample_overlay_start(FH_VOID)
{
	...
	FHAdv_Osd_Config_GboxPixel(FH_APP_GRP_ID, 2, FHTEN_OSD_GBOX_Pixel);
...
{
    FHAdv_Osd_UnInit(FH_APP_GRP_ID);
}
