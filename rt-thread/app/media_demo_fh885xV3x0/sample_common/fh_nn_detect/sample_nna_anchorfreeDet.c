...
static FH_VOID draw_box(FH_DETECTION_T *out, FH_UINT32 box_chnw, FH_UINT32 box_chnh)
{
    ...
    {
        /*draw gosd rect*/
        if (out->detBBox[i].bbox.x < 0)
        {
            out->detBBox[i].bbox.x = 0;
        }
        if (out->detBBox[i].bbox.y < 0)
        {
            out->detBBox[i].bbox.y = 0;
        }
        if (out->detBBox[i].bbox.w < 0)
        {
            out->detBBox[i].bbox.w = 0;
        }
        if (out->detBBox[i].bbox.h < 0)
        {
            out->detBBox[i].bbox.h = 0;
        }
        if (out->detBBox[i].bbox.x > 1)
        {
            out->detBBox[i].bbox.x = 0.99;
        }
        if (out->detBBox[i].bbox.y > 1)
        {
            out->detBBox[i].bbox.y = 0.99;
        }
        if (out->detBBox[i].bbox.w > 1)
        {
            out->detBBox[i].bbox.w = 0.99;
        }
        if (out->detBBox[i].bbox.h > 1)
        {
            out->detBBox[i].bbox.h = 0.99;
        }
...
static FH_VOID *sample_init_fh_nn(FH_VOID)
{
    ...
    nna_param.type = FN_DET_DET_ANCHORFREE;
    strcpy(modelPath, "./anchorfree/case20024_det_anchorfree.nbg");
...
/* 512 * 288 */
FH_VOID *thread_nn_anchorfreeDet(FH_VOID *args)
{
    FH_SINT32 ret;
    FH_VOID *detHandle = NULL;
    FH_VPU_STREAM_ADV rgbStr;
    FH_QTENSOR_ARRAY_T out_info;
    ...
    while (!g_stop_nna_detect)
    {
        ...
        ret = FH_NNA_DET_CUSTOM_Process(detHandle, &nn_src, &out_info);
        if (ret)
        {
            printf("Error[FH_NN]: FH_NNA_DET_CUSTOM_Process failed, ret = %x\n", ret);
            FH_NNA_DET_CUSTOM_Release(detHandle);
            continue;
        }

        ret = FH_NNA_DET_CUSTOM_POST_Process(detHandle, &out_info, &nn_out);
        if (ret)
        {
            printf("Error[FH_NN]: FH_NNA_DET_CUSTOM_POST_Process failed, ret = %x\n", ret);
            FH_NNA_DET_CUSTOM_Release(detHandle);
            continue;
        }

        ret = FH_NNA_DET_CUSTOM_Release(detHandle);
        if(ret)
        {
            printf("Error[FH_NN]: FH_NNA_DET_CUSTOM_Release failed, ret = %x\n", ret);
        }
...
FH_SINT32 sample_fh_nn_anchorfreeDet_start(FH_VOID)
{
    pthread_t nn_thread;
...
#endif
    if (pthread_create(&nn_thread, &nn_attr, thread_nn_anchorfreeDet, NULL))
    {
...
FH_SINT32 sample_fh_nn_anchorfreeDet_stop(FH_VOID)
{
    g_stop_nna_detect = 1;
