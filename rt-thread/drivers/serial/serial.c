...
rt_inline int _serial_int_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    /* If no data in FiFo - Suspend songyh@18.3.19 */
    if (rx_fifo->get_index == rx_fifo->put_index)
    {
        rt_completion_init(&rx_fifo->completion);
        rt_completion_wait(&rx_fifo->completion, 1);
    }
                                 ...
                                 rt_size_t         size)
{
    ...
    if (serial->ops->putmulti && rt_console_get_device() == dev)
    {
        return serial->ops->putmulti(serial, buffer, size);
    }
...
void rt_hw_serial_isr(struct rt_serial_device *serial, int event)
{
    switch (event & 0xff)
    {
        case RT_SERIAL_EVENT_RX_IND:
        {
            ...
            /* FiFo is Ready Pepare to release resource songyh@18.3.19 */
            rt_completion_done(&(rx_fifo->completion));
            break;
        }
