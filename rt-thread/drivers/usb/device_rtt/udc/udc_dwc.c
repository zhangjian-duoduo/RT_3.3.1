...
void dwc_handle_ep_data_in_phase(dwc_handle *dwc, uint8_t epnum)
{
    ...
    if (dwc->is_dma != 0)
    {
        ...
        REG_DIEP_DMA(epnum) = (uint32_t)(pep->xfer_buff);
...
int HW_SendPKT(dwc_handle *dwc, uint8_t epnum, const uint8_t *buf, int size)
{
    ...
    /*pep->xfer_buff  = (uint8_t *)buf;*/
    if (size > 0)
    {
        memcpy(pep->xfer_buff, buf, size);
        mmu_clean_invalidated_dcache((rt_uint32_t)pep->xfer_buff, size);
    }
...
static void dwc_calculate_fifo_size(dwc_handle *dwc)
{
    ...
    np_txfifo_size = ((1 + 1) * (64 / 4));
    REG_GNPTXFIFO_SIZE = (np_txfifo_size << 16) | startaddr;

#define DWC_TX_FIFO_SIZE ((1 + 1) * (512 / 4))
    startaddr = np_txfifo_size + rx_fifo_size;
    for (i = 1; i < dwc->hwcfg4.b.num_in_eps-1; i++)
    {
        if (i < 7)
            tx_fifo_size = DWC_TX_FIFO_SIZE;
        else
            tx_fifo_size = DWC_TX_FIFO_SIZE / 16;
...
void dwc_ep0_in_intr(dwc_handle *dwc, uint8_t epnum)
{
    ...
    rt_kprintf("REG_DIEP_DMA 0: 0x%x\n", REG_DIEP_DMA(0));
    ...
    if (intr & DEP_XFER_COMP)
    {
        ...
        case EP_DATA:
            ...
            {
                udc_usbd_event_cb(0, USB_EVT_IN, RT_NULL);
            }
            else
#endif
            {
...
static void dwc_otg_device_init(dwc_handle *dwc)
{
    ...
    /* Program the GINTMSK */
    REG_GINT_MASK |= GINTMSK_IEP_INTR | GINTMSK_OEP_INTR |GINTMSK_USB_RESET | GINTMSK_ENUM_DONE |
                     GINTMSK_USB_EARLYSUSPEND | GINTMSK_USB_SUSPEND | (1 <<  31);
}
...
static void dwc_otg_core_init(dwc_handle *dwc, uint8_t dma_enable)
{
    ...
    else if (dwc->is_dma == IS_INTERN_DMA)
    {
        if (dma_enable)
        {
            ...
            ahbcfg |= AHBCFG_DMA_ENA | (DWC_GAHBCFG_INT_DMA_BURST_INCR4 << 1);
        }
        else
        {
