...
static struct ep_id __ep_pool[] = {
    ...
    {0x02, USB_EP_ATTR_INT,         USB_DIR_IN,    64, ID_UNASSIGNED},
#endif
#ifdef FH_RT_USB_DEVICE_UVC
#ifdef FH_RT_UVC_EP_TYPE_ISOC
    {UVC_STREAM1_EP1, USB_EP_ATTR_ISOC,        USB_DIR_IN,   UVC_PACKET_SIZE, ID_UNASSIGNED},
#ifdef UVC_DOUBLE_STREAM
    {UVC_STREAM2_EP1, USB_EP_ATTR_ISOC,        USB_DIR_IN,   UVC_PACKET_SIZE, ID_UNASSIGNED},
#endif
#else
    {UVC_STREAM1_EP1, USB_EP_ATTR_BULK,        USB_DIR_IN,   UVC_PACKET_SIZE, ID_UNASSIGNED},
#ifdef UVC_DOUBLE_STREAM
    {UVC_STREAM2_EP1, USB_EP_ATTR_BULK,        USB_DIR_IN,   UVC_PACKET_SIZE, ID_UNASSIGNED},
#endif
#endif
#endif
#ifdef FH_RT_USB_DEVICE_CDC
    {USB_CDC_VCOM_EP, USB_EP_ATTR_BULK,        USB_DIR_OUT,     512, ID_UNASSIGNED},
    {USB_CDC_VCOM_EP, USB_EP_ATTR_BULK,        USB_DIR_IN,     512, ID_UNASSIGNED},
#endif

#ifdef FH_RT_USB_DEVICE_HID
    {USB_HID_EP, USB_EP_ATTR_INT,        USB_DIR_OUT,     64, ID_UNASSIGNED},
    {USB_HID_EP, USB_EP_ATTR_INT,        USB_DIR_IN,     64, ID_UNASSIGNED},
#endif

#ifdef FH_RT_USB_DEVICE_MSTORAGE
    {USB_MSTORAGE_EP, USB_EP_ATTR_BULK,        USB_DIR_OUT,     512, ID_UNASSIGNED},
    {USB_MSTORAGE_EP, USB_EP_ATTR_BULK,        USB_DIR_IN,     512, ID_UNASSIGNED},
#endif

#ifdef FH_RT_USB_DEVICE_UAC
    {USB_UAC_EP, USB_EP_ATTR_ISOC,        USB_DIR_IN,    256, ID_UNASSIGNED},
    {USB_UAC_EP, USB_EP_ATTR_ISOC,        USB_DIR_OUT,    256, ID_UNASSIGNED},
#endif
#ifdef FH_RT_USB_DEVICE_RNDIS
    {USB_RNDIS_EP_INT, USB_EP_ATTR_INT,        USB_DIR_IN,    8, ID_UNASSIGNED},
    {USB_RNDIS_EP, USB_EP_ATTR_BULK,        USB_DIR_IN,    512, ID_UNASSIGNED},
    {USB_RNDIS_EP, USB_EP_ATTR_BULK,        USB_DIR_OUT,    512, ID_UNASSIGNED},
#endif
...
static rt_err_t __ep_enable(uep_t ep)
{
    ...
    {
        __dwc_hdl.dep[DWC_EP_IN_OFS + ep->id->addr]->maxpacket = EP_MAXPACKET(ep);
...
static rt_err_t __ep_disable(uep_t ep)
{
    ...
    /* if (ep->id->dir == USB_DIR_IN)
    {
        ...
        dwc_disable_in_ep(&__dwc_hdl, ep->id->addr);
    } */
...
static rt_err_t __init(rt_device_t device)
{
    ...
    for (epidx = 0; __ep_pool[epidx].addr != 0xFF; ++epidx)
    {
        ...
        rt_uint8_t  *pXfer_align = RT_NULL;
        ...
        { /* EP0 is IN-OUT */
            ...
            pXfer = rt_malloc_align(__ep_pool[epidx].maxpacket * 2, CACHE_LINE_SIZE);
            ...
            pXfer_align = rt_malloc_align(4096, CACHE_LINE_SIZE);
            if (!pXfer_align)
            {
                rt_kprintf("ERROR: no memory for pXfer\n");
                while (1)
            ...
            if (pep)
            {
                ...
                pep->data_buff_align = (void *)pXfer_align;
                pep->xfer_len   = 0;
                pep->xfer_count = 0;
            }
