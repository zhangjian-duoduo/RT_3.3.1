...
struct fh_uart uart0 = {(uart *)UART0_REG_BASE, UART0_IRQn, 0, UART0_RX, UART0_TX};
#endif

#if defined(FH_USING_UART1)
static struct rt_serial_device serial1;
struct fh_uart uart1 = {(uart *)UART1_REG_BASE, UART1_IRQn, 0, UART1_RX, UART1_TX};

#endif

#if defined(FH_USING_UART2)
static struct rt_serial_device serial2;
struct fh_uart uart2 = {(uart *)UART2_REG_BASE, UART2_IRQn, 1, UART2_RX, UART2_TX};
#endif

#if defined(FH_USING_UART3)
static struct rt_serial_device serial3;
struct fh_uart uart3 = {(uart *)UART3_REG_BASE, UART3_IRQn, 1, UART3_RX, UART3_TX};
#endif
...
rt_uint32_t uart_dma_rx_available(void)
{
    rt_uint32_t write_ptr = 0;
    rt_uint32_t curr_dar = 0;
    /* calc write ptr */
    curr_dar = uart_dma->dma_dev->ops->control(uart_dma->dma_dev,
            RT_DEVICE_CTRL_DMA_GET_DAR,
           (void *)uart_dma->dma_rx);
    write_ptr = curr_dar - (rt_uint32_t)uart_dma->dma_buff_rx;

    PRINT_UART_DBG("write ptr: %d\n", write_ptr);

    /* any data available? */
    if(!uart_dma->ptr_on_diff_page)
    {
        if(write_ptr >= uart_dma->rx_buf_read_ptr)
            return write_ptr - uart_dma->rx_buf_read_ptr;
    }
    else if(uart_dma->ptr_on_diff_page == 1)
    {
        PRINT_UART_DBG("aval: w%d, r%d\n", write_ptr, uart_dma->rx_buf_read_ptr);
        if(write_ptr <= uart_dma->rx_buf_read_ptr)
            return UART_DMA_RXMEM_SIZE - uart_dma->rx_buf_read_ptr + write_ptr;
    }
...
{
    int available = uart_dma_rx_available();
    int leftover = 0;

    PRINT_UART_DBG("available: %d\n", available);

    if(available >= 0)
    {
        available = MIN(available, data_size);
        if((available + uart_dma->rx_buf_read_ptr) <= UART_DMA_RXMEM_SIZE)
        {
            rt_memcpy((void *)mem_addr,
                    (void *)uart_dma->dma_buff_rx + uart_dma->rx_buf_read_ptr,
                    available);
            uart_dma->rx_buf_read_ptr += available;

            if(uart_dma->rx_buf_read_ptr == UART_DMA_RXMEM_SIZE)
            {
                uart_dma->rx_buf_read_ptr = 0;
                uart_dma->ptr_on_diff_page--;
            }
        }
        else
        {
            leftover = UART_DMA_RXMEM_SIZE - uart_dma->rx_buf_read_ptr;
            rt_memcpy((void *)mem_addr,
                    (void *)uart_dma->dma_buff_rx + uart_dma->rx_buf_read_ptr,
                    leftover);
            rt_memcpy((void *)(mem_addr + UART_DMA_RXMEM_SIZE - uart_dma->rx_buf_read_ptr - 1),
                    (void *)uart_dma->dma_buff_rx,
                    available - leftover);
            uart_dma->ptr_on_diff_page--;
            uart_dma->rx_buf_read_ptr = available - leftover;
        }
...
static void uart_dma_rx_done(void *arg)
{
    ...
    if(uart_dma->ptr_on_diff_page++ > 0)
        uart_dma->ptr_on_diff_page = 1;
    PRINT_UART_DBG("kick\n");
}
