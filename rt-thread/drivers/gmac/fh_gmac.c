...
int fh_gmac_init(rt_device_t dev)
{
    ...
    if (gmac->p_plat_data->early_func)
    {
        gmac->p_plat_data->early_func(gmac->p_plat_data->p_early);
    }
...
int fh_gmac_probe(void *priv_data)
{
    ...
    gmac->speed_switch_pmu_reg = p_plat->speed_switch_pmu_reg;
    gmac->speed_switch_bit_pos = p_plat->speed_switch_bit_pos;
    gmac->speed_switch_100M_val = p_plat->speed_switch_100M_val;
