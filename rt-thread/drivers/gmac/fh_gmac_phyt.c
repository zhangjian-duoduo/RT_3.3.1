...
int fh_gmac_phy_reset(fh_gmac_object_t *pGmac, unsigned int phy_sel);
...
void external_phy_pin_sel(fh_gmac_object_t *gmac, int flag)
{
    ...
    if (flag)
    {
        for (i = 0; i < ARRAY_SIZE(_mdio_pin_ref_obj); i++)
        {
            ...
            else
            {
                ...
                if (strcmp(_mdio_pin_ref_obj[i].old,
                "MAC_RMII_CLK") == 0)
                    fh_pinctrl_smux("RMII", "MAC_REF_CLK", 0, 0);
            }
...
int auto_find_phy(fh_gmac_object_t *gmac)
{
    ...
    c_driver != 0; i++, c_driver = phy_driver_array[i]) {
        ...
        if (strcmp(c_driver, "FH EPHY") == 0)
        {
            ...
            fh_pmu_inf_set(INTERNAL_PHY);
            fh_gmac_phy_reset(gmac, INTERNAL_PHY);
            gmac->phy_sel = INTERNAL_PHY;
        }
        else
        {
            external_phy_pin_sel(gmac, 1);
            fh_pmu_inf_set(EXTERNAL_PHY);
            switch_flag = 1;
            fh_pmu_ephy_sel(EXTERNAL_PHY);
            fh_gmac_phy_reset(gmac, EXTERNAL_PHY);
    ...
    if (strcmp(c_driver, "Generic PHY") == 0)
        fh_pinctrl_sdev("RMII", 0x8); /* no print */
...
#if defined(CONFIG_ARCH_FH885xV200) || \
defined(CONFIG_ARCH_FH865x) || defined(CONFIG_ARCH_FH8636_FH8852V20X) || \
defined(CONFIG_ARCH_FH885xV300) || defined(CONFIG_ARCH_FH885xV310)
    if (phy_sel == EXTERNAL_PHY)
    {
        gpio_request(pGmac->phyreset_gpio);
        gpio_direction_output(pGmac->phyreset_gpio, 1);
        gpio_direction_output(pGmac->phyreset_gpio, 0);
        /* actual is 20ms */
        rt_thread_delay(2);
        gpio_direction_output(pGmac->phyreset_gpio, 1);
        rt_thread_delay(2);
        gpio_release(pGmac->phyreset_gpio);
        rt_thread_delay(1);
        return 0;
    }
...
int fh_mdio_set_mii(fh_gmac_object_t* pGmac)
{
    ...
    if (pGmac->phy_interface == PHY_INTERFACE_MODE_RMII)
    {
        ...
        case FH_GMAC_PHY_RTL8201:
            ...
            fh_gmac_set_phy_register(pGmac, 0x0d, 0x7);
            fh_gmac_set_phy_register(pGmac, 0x0e, 0x3c);
            fh_gmac_set_phy_register(pGmac, 0x0d, 0x4007);
            fh_gmac_set_phy_register(pGmac, 0x0e, 0x0);
            break;
        case FH_GMAC_PHY_IP101G:
            ...
            fh_gmac_set_phy_register(pGmac, 26, 0xc5ed);
#endif
            fh_gmac_set_phy_register(pGmac,
                    gmac_phyt_ip101g_page_select, 0x0);
        ...
        case FH_GMAC_PHY_TI83848:
            rmii_mode = fh_gmac_get_phy_register(pGmac,
        ...
        case FH_GMAC_PHY_INTERNAL:
            rt_kprintf("found fh internal phy...\n");

            /* switch to page 1 */
            fh_gmac_set_phy_register(pGmac, 0x1f, 0x100);
            /* Disable APS */
            fh_gmac_set_phy_register(pGmac, 0x12, 0x4824);
            /*switch to page 2*/
            fh_gmac_set_phy_register(pGmac, 0x1f, 0x200);
            /* PHY AFE TRX optimization */
            fh_gmac_set_phy_register(pGmac, 0x18, 0);
            /* switch to page 6 */
            fh_gmac_set_phy_register(pGmac, 0x1f, 0x600);
            /* PHY AFE ADC optimization */
            fh_gmac_set_phy_register(pGmac, 0x10, 0x555b);
            /* PHYAFE TX optimization */
            fh_gmac_set_phy_register(pGmac, 0x14, 0x708f);
            /* CP current optimization */
            fh_gmac_set_phy_register(pGmac, 0x17, 0x575);
            /* ADC OP BIAS optimization */
            fh_gmac_set_phy_register(pGmac, 0x18, 0);
            /* RX signal detector level optimization */
            fh_gmac_set_phy_register(pGmac, 0x19, 0x408);
            /* PHY AEF PDCW optimization */
            fh_gmac_set_phy_register(pGmac, 0x1c, 0x8880);

            /*drive set below.*/
            /*first debug open*/
            fh_gmac_set_phy_register(pGmac, 0x1f, 0x800);
            fh_gmac_set_phy_register(pGmac, 0x1d, 0x844);
            fh_gmac_set_phy_register(pGmac, 0x1f, 0x600);
            fh_gmac_set_phy_register(pGmac, 0x15, 0x1320);

            fh_gmac_set_phy_register(pGmac, 0x1f, 0x600);
            ret = fh_gmac_get_phy_register(pGmac, 21);
            ret &= ~(0x1f << 8 | 0x1f);
            ret |= 0x10 << 8 | 0x1f | 1 << 13;
            fh_gmac_set_phy_register(pGmac, 21, ret);

            /*read back debug data*/
            fh_gmac_set_phy_register(pGmac, 0x1f, 0x800);
            ret = fh_gmac_get_phy_register(pGmac, 0x1e);
            rt_kprintf("AFE driver [Raw] : [100M] : [10M] = [%x] : [%x] : [%x]\n",
            ret, (ret >> 8) & 0x1f, (ret & 0x1f));

            fh_gmac_set_phy_register(pGmac, 0x1f, 0);
            fh_gmac_set_phy_register(pGmac, 0x0d, 0x7);
            fh_gmac_set_phy_register(pGmac, 0x0e, 0x3c);
            fh_gmac_set_phy_register(pGmac, 0x0d, 0x4007);
            fh_gmac_set_phy_register(pGmac, 0x0e, 0x0);
            ...
            break;
#if defined CONFIG_ARCH_FH8636_FH8852V20X || defined CONFIG_ARCH_FH885xV300 || defined CONFIG_ARCH_FH885xV310
        case FH_GMAC_PHY_INTERNAL_V2:
            rt_kprintf("found fh ephy_v2\n");
        break;
#endif
        }
...
static void fh_set_rmii_speed(fh_gmac_object_t *pGmac, int speed)
{
    ...
    reg = fh_pmu_get_reg(pGmac->speed_switch_pmu_reg);
    reg &= ~(1 << pGmac->speed_switch_bit_pos);
    if (speed == gmac_speed_10m)
        reg |= (!pGmac->speed_switch_100M_val) << pGmac->speed_switch_bit_pos;
    else
        reg |= pGmac->speed_switch_100M_val << pGmac->speed_switch_bit_pos;
    fh_pmu_set_reg(pGmac->speed_switch_pmu_reg, reg);
}
    ...
    if (phydev->link)
    {
        ...
        */
        ...
        if (phydev->speed != pGmac->speed)
        {
            ...
            case 100:
                ctrl |= 0x4000;
                fh_set_rmii_speed(pGmac, gmac_speed_100m);
                break;
            case 10:
                ctrl &= ~0x4000;
                fh_set_rmii_speed(pGmac, gmac_speed_10m);
                break;
            default:
