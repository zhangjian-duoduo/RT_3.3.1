...
{
    struct rt_mmc_ext_csd *card_ext_csd = RT_NULL;
    ...
    card_ext_csd = &card->ext_csd;
    card->flags |=  CARD_FLAG_HIGHSPEED;
    card->hs_max_data_rate = 200000000;

    card_ext_csd->sectors = ext_csd[EXT_CSD_SEC_CNT + 0] << 0 |
                            ext_csd[EXT_CSD_SEC_CNT + 1] << 8 |
                            ext_csd[EXT_CSD_SEC_CNT + 2] << 16 |
                            ext_csd[EXT_CSD_SEC_CNT + 3] << 24;

    card->card_capacity = (unsigned long long)card_ext_csd->sectors * 512 >> 10;
    rt_kprintf("emmc card capacity %d MB\n", card->card_capacity>>10);
                      ...
                      rt_uint8_t index, rt_uint8_t value)
{
    ...
    do
    {
        ...
        if (err)
        {
            rt_kprintf("world [sdio]:\terror %d requesting status\n", err);
            break;
        }
                                     ...
                                     rt_uint32_t           ocr)
{
    ...
    */
    ...
    err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
                     ...
                     0x01);
    if (err)
       rt_kprintf("mmc_switch EXT_CSD_HS_TIMING fail\n");
