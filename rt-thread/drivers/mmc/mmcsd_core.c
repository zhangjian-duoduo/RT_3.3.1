...
typedef struct {
    mmc_dectect_info info;
    rt_uint32_t flag;
}st_mmc_dectect_info_pool;
...
static rt_uint32_t mmcsd_detect_mb_pool[16];
static struct rt_mailbox mmcsd_hotpluge_mb;
static rt_uint32_t mmcsd_hotpluge_mb_pool[16];
static st_mmc_dectect_info_pool mmc_dectect_info_pool[16];

static mmc_dectect_info *alloc_mmc_dectect_info(void)
{
    rt_uint32_t i = 0;
    mmc_dectect_info *info = RT_NULL;
    for (i = 0; i < sizeof(mmc_dectect_info_pool)/sizeof(st_mmc_dectect_info_pool); i++)
    {
        if (mmc_dectect_info_pool[i].flag == 0)
        {
            info = &mmc_dectect_info_pool[i].info;
            mmc_dectect_info_pool[i].flag = 1;
            break;
        }
    }
    return info;
}
static void free_mmc_dectect_info(mmc_dectect_info *info)
{
    rt_uint32_t i = 0;
    for (i = 0; i < sizeof(mmc_dectect_info_pool)/sizeof(st_mmc_dectect_info_pool); i++)
    {
        if (&mmc_dectect_info_pool[i].info == info)
        {
            mmc_dectect_info_pool[i].flag = 0;
            break;
        }
    }
    return;
...
{
    mmc_dectect_info *mmcinfo = alloc_mmc_dectect_info();
    if (mmcinfo != RT_NULL)
    {
        mmcinfo->host             = (rt_uint32_t)host;
        mmcinfo->pluged           = RT_TRUE;
        mmcinfo->needfree         = RT_TRUE;
        rt_mb_send(&mmcsd_detect_mb, (rt_uint32_t)mmcinfo);
    }
    else
    {
        LOG_E("mmc_dectect_info pool full\n");
    }
}

void mmcsd_change_local(struct rt_mmcsd_host *host, rt_uint32_t pluged)
{
    mmc_dectect_info *mmcinfo = alloc_mmc_dectect_info();
    if (mmcinfo != RT_NULL)
    {
        mmcinfo->host             = (rt_uint32_t)host;
        mmcinfo->pluged           = pluged;
        mmcinfo->needfree         = RT_TRUE;
        rt_mb_send(&mmcsd_detect_mb, (rt_uint32_t)mmcinfo);
    }
    else
...
void mmcsd_init_ctl(struct rt_mmcsd_host *host)
{
    rt_mmcsd_ctldev_probe(host);
}
#endif
void mmcsd_detect(void *param)
{
    ...
    rt_uint32_t ocr;
    rt_int32_t err;
    mmc_dectect_info *mmcinfo = RT_NULL;

    while (1) 
    {
        if (rt_mb_recv(&mmcsd_detect_mb, (rt_uint32_t *)&mmcinfo, RT_WAITING_FOREVER) == RT_EOK)
        {
            if (mmcinfo == RT_NULL)
                continue;
            host = (struct rt_mmcsd_host *)mmcinfo->host;
            if (host->card == RT_NULL && mmcinfo->pluged == RT_TRUE)
            {
                ...
                if (!err)
                {
                    ...
                    if (mmcinfo->needfree == RT_TRUE)
                    {
                        free_mmc_dectect_info(mmcinfo);
                        mmcinfo = RT_NULL;
                    }
                    continue;
                }
            ...
            else if (mmcinfo->pluged == RT_FALSE && host->card != RT_NULL)
            {
                mmcsd_host_lock(host);
...
void rt_mmcsd_core_init(void)
{
    ...
    rt_memset(&mmc_dectect_info_pool, 0, sizeof(mmc_dectect_info_pool));
