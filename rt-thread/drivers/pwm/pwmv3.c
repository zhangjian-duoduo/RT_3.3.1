 ...
 * Change Logs:
 * Date           Author       Notes
 * 2020-04-01     songyh    the first version
 *
 */
...
#define OFFSET_PWM_BASE(n)         (0x80 + 0x80 * n)
...
/****************************************************
 函数名： fh_pwm_inverse
 函数功能：PWM模块输出为正向或者是反向
 输入参数： n:表示输入PWM通道号；inverse：0表示输出正向，1表示输出反向
 输出参数：无
 返回值定义：0：配置成功；非0：配置失败
 ****************************************************/
static unsigned int fh_pwm_inverse(unsigned int n, unsigned int inverse)
{
    unsigned int reg;
...
static rt_err_t fh_pwm_set_config(struct fh_pwm_chip_data *chip_data)
{
    ...
    unsigned int period, duty, delay, phase, inverse;
    ...
    /* config out inverse or not */
    fh_pwm_inverse(chip_data->id, inverse);
    /* config out delay */
...
void fh_pwm_interrupt(int vector, void *param)
{
    ...
    while (stat_once)
    {
        irq_once = fls(stat_once);
        chip_data = (&fh_pwm_drv->pwm[irq_once - 1]);
        if (chip_data && chip_data->finishonce_callback)
            chip_data->finishonce_callback(chip_data);
        fh_pwm_interrupt_finishonce_clear(irq_once - 1);
        stat_once &= ~(1 << (irq_once - 1));
    }

    while (stat_all)
    {
        irq_all = fls(stat_all);
        chip_data = (&fh_pwm_drv->pwm[irq_all - 1]);
        if (chip_data && chip_data->finishall_callback)
            chip_data->finishall_callback(chip_data);
        fh_pwm_interrupt_finishall_clear(irq_all - 1);
        stat_all &= ~(1 << (irq_all - 1));
    }
