...
#if defined(FH_I2C_DEBUG) && defined(RT_DEBUG)
...
/* struct rt_i2c_msg *pmsg = RT_NULL; */
    ...
    {
        PRINT_I2C_DBG("ERROR: %s, i2c_priv>cmd_err == DW_IC_ERR_TX_ABRT\n",
                   __func__);
...
static void i2c_fh_xfer_msg(struct rt_i2c_bus_device *dev)
{
    ...
    PRINT_I2C_DBG("%s start, msgs_num: %d, write_idx: %d\n", __func__,
                  i2c_drv->msgs_num, i2c_drv->msg_write_idx);
    ...
    for (; i2c_drv->msg_write_idx < i2c_drv->msgs_num; i2c_drv->msg_write_idx++)
    {
        ...
        {
            rt_kprintf("ERROR: %s, invalid target address\n", __func__);
            i2c_drv->msg_err = 1;
            break;
        }
        ...
        {
            rt_kprintf("ERROR: %s, invalid message length\n", __func__);
            i2c_drv->msg_err = 1;
            break;
        }
        ...
        tx_limit =
            i2c_obj->config.tx_fifo_depth - I2C_GetTransmitFifoLevel(i2c_obj);
        rx_limit =
            i2c_obj->config.rx_fifo_depth - I2C_GetReceiveFifoLevel(i2c_obj);
        ...
        while (buf_len > 0 && tx_limit > 0 && rx_limit > 0)
        {
            ...
            if (!buf_len &&
                ((i2c_drv->msg_write_idx == i2c_drv->msgs_num - 1) ||
                (msgs[i2c_drv->msg_write_idx].flags & RT_I2C_SEPRATE_MSG)))
            {
                /* 2015-11-8 ar0130 bug fixed */
                /* 20*1000 about 2 *0.1 ms(for i2c send one byte @ 100KHz)
                 * fixme: define MACRO get timeout value; */
                unsigned int _timeout = 20000;
                while (I2C_GetTransmitFifoLevel(i2c_obj) && _timeout--)
                    ;
                cmd |= 0x200;
            }
...
static void i2c_fh_read(struct rt_i2c_bus_device *dev)
{
    ...
    PRINT_I2C_DBG("%s start, msgs_num: %d, read_idx: %d\n", __func__,
                  i2c_drv->msgs_num, i2c_drv->msg_read_idx);
    ...
    for (; i2c_drv->msg_read_idx < i2c_drv->msgs_num; i2c_drv->msg_read_idx++)
    {
        ...
        PRINT_I2C_DBG("i2c_fh_read, len: %d, buf[0]: 0x%x\n",
                      msgs[i2c_drv->msg_read_idx].len,
                      msgs[i2c_drv->msg_read_idx].buf[0]);
...
int fh_i2c_probe(void *priv_data)
{
    ...
    char i2c_clk_name[10] = {0};
    char i2c_isr_name[10] = {0};

    PRINT_I2C_DBG("%s start\n", __func__);

    i2c_bus_dev =
        (struct rt_i2c_bus_device *)rt_malloc(sizeof(struct rt_i2c_bus_device));
    rt_memset(i2c_bus_dev, 0, sizeof(struct rt_i2c_bus_device));
    i2c_bus_dev->ops = &fh_i2c_ops;

    rt_snprintf(i2c_dev_name, sizeof(i2c_dev_name), "%s%d", "i2c", i2c_obj->id);
    rt_snprintf(i2c_clk_name, sizeof(i2c_clk_name), I2C_CLK_NAME, i2c_obj->id);
    rt_snprintf(i2c_isr_name, sizeof(i2c_isr_name), I2C_ISR_NAME, i2c_obj->id);
    ...
    rt_hw_interrupt_install(i2c_obj->irq, fh_i2c_interrupt, (void *)i2c_drv,
                            i2c_isr_name);
    clk_rate = clk_get(NULL, i2c_clk_name);
    ...
    {
        clk_enable(clk_rate);
        i2c_obj->input_clock = clk_rate->clk_out_rate/1000;
    }
...
int fh_i2c_exit(void *priv_data) { return 0; }
struct fh_board_ops i2c_driver_ops = {
    .probe = fh_i2c_probe, .exit = fh_i2c_exit,
};
...
#if 0       /* debug code, enable only when debug */
...
void i2c_test_sensor(void)
{
    struct rt_i2c_bus_device *fh81_i2c;
/* struct rt_i2c_msg msg[2]; */
    rt_uint8_t data[1] = {0x00};

    fh81_i2c = rt_i2c_bus_device_find("i2c0");

    fh_i2c_write_reg(fh81_i2c, 0x12, 0x30);

    fh_i2c_read_reg(fh81_i2c, 0x0c, data);

    rt_kprintf("data read from 0x0c is 0x%x\r\n", data[0]);
    PRINT_I2C_DBG("%s end\n", __func__);
}
