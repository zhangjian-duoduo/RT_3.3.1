...
int I2C_WaitMasterIdle(struct fh_i2c_obj *i2c_obj)
{
    ...
    while (GET_REG(i2c_obj->base + OFFSET_I2C_STATUS) &
           DW_IC_STATUS_MASTER_ACTIVITY)
    {
        if (timeout < 0)
        {
            rt_kprintf(
                "ERROR: %s, timeout waiting for master not active, txflr: 0x%x, rxflr: 0x%x, stat: 0x%x\n",
                __func__, I2C_GetReceiveFifoLevel(i2c_obj),
                I2C_GetTransmitFifoLevel(i2c_obj),
                GET_REG(i2c_obj->base + OFFSET_I2C_INTR_STAT));
            return -RT_ETIMEOUT;
        }
        udelay(10);
        timeout--;
...
static inline UINT32 I2C_CalcSclHcnt(UINT32 ic_clk, UINT32 tSYMBOL, UINT32 tf,
                                     int cond, int offset)
{
    /*
...
static inline UINT32 I2C_CalcSclLcnt(UINT32 ic_clk, UINT32 tLOW, UINT32 tf,
                                     int offset)
{
    /*
     * Conditional expression:
...
static int I2C_SetSpeedCount(struct fh_i2c_obj *i2c_obj)
{
    ...
    /* Standard-mode */
    hcnt =
        I2C_CalcSclHcnt(i2c_obj->input_clock, 40, /* tHD;STA = tHIGH = 4.0 us */
                        3,                        /* tf = 0.3 us */
                        0,                        /* 0: DW default, 1: Ideal */
                        0);                       /* No offset */
    lcnt = I2C_CalcSclLcnt(i2c_obj->input_clock, 47, /* tLOW = 4.7 us */
                           3,                        /* tf = 0.3 us */
                           0);                       /* No offset */
    ...
    /* Fast-mode */
    hcnt =
        I2C_CalcSclHcnt(i2c_obj->input_clock, 6, /* tHD;STA = tHIGH = 0.6 us */
                        3,                       /* tf = 0.3 us */
                        0,                       /* 0: DW default, 1: Ideal */
                        0);                      /* No offset */
    lcnt = I2C_CalcSclLcnt(i2c_obj->input_clock, 13, /* tLOW = 1.3 us */
                           3,                        /* tf = 0.3 us */
                           0);                       /* No offset */
...
UINT32 I2C_ClearAndGetInterrupts(struct fh_i2c_obj *i2c_obj)
{
    ...
    if (stat & DW_IC_INTR_TX_ABRT)
    {
         ...
         */
        i2c_obj->abort_source =
            GET_REG(i2c_obj->base + OFFSET_I2C_TX_ABRT_SOURCE);
...
void I2C_Init(struct fh_i2c_obj *i2c_obj)
{
    ...
    SET_REG(i2c_obj->base + OFFSET_I2C_TX_TL,
            i2c_obj->config.tx_fifo_depth - 1);
    SET_REG(i2c_obj->base + OFFSET_I2C_RX_TL, 0);

    /* configure the i2c master */
    ic_con =
        DW_IC_CON_MASTER | DW_IC_CON_SLAVE_DISABLE |
