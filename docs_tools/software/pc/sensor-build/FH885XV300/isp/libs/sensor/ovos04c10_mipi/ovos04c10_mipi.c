...
#include "isp_enum.h"
#include "mipi_api.h"
#include "ovos04c10_mipi_param.h"
#include "ovos04c10_mipi_wdr_param.h"
#include "sensor.h"
#include "ovos04c10_mipi.h"

#define _NAME(n, s) n##_##s
#define NAME(n) _NAME(n, ovos04c10_mipi)
...
static int Set_short_exp_intt(void* pvObj, FH_UINT32 intt)
{
    ...
    if (intt <= 8)
    {
        Sensor_Write(pstCci, 0x5142, 0x04);
        Sensor_Write(pstCci, 0x5143, 0xa0);
    }
    else if (intt > 16)
    {
        Sensor_Write(pstCci, 0x5142, 0x04);
        Sensor_Write(pstCci, 0x5143, 0x00);
    }
    return 0;
}
...
static int SetSensorFlipMirror(void* pvObj, FH_UINT32 sensor_en_stat)
{
    ...
    tmp1 = Sensor_Read(pstCci, 0x3820);  // bit[3]:mirror  bit[4]:flip
    tmp1 = (tmp1&0xffc7) | ((1-mirror) << 3) | (flip << 4);
    Sensor_Write(pstCci, 0x3820, tmp1);
    Sensor_Write(pstCci, 0x3716, Reg3716);
    return 0;
}
...
static int GetSensorFlipMirror(void* pvObj, FH_UINT32 *sensor_en_stat)
{
    ...
    mirror = 1 - ((Sensor_Read(pstCci, 0x3820)>>3) & 0x1);
    flip = (Sensor_Read(pstCci, 0x3820)>>4) & 0x1;
...
static FH_UINT32* GetMirrorFlipBayerFormat(void* pvObj){
    SNS_CTRL_PARA_S*    pstCtrlPara = _sns_get_ctrl_para(pvObj);
    if (pstCtrlPara->u32WdrFlag)
        return MirrorFlipBayerFormatWdr;
    else
...
#define SENSOR_OVOS04C10_MIPI    "ovos04c10_mipi"
struct isp_sensor_if ovos04c10_mipi_sensor_if = {
    .name = SENSOR_OVOS04C10_MIPI,
...
struct isp_sensor_if *NAME(Sensor_Create)()
{
    SensorCreateInfo info = {
        .pstSnsIf       = &ovos04c10_mipi_sensor_if,
